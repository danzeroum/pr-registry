...
> **Status:** Fases 1-4 concluídas. O produto pivotou na Fase 3 para uma
...
> Nada das fases anteriores foi descartado: o motor está em `core/`,
> compartilhado por `action/` (modo alternativo) e `service/` (modo
> principal). A Fase 4 adiciona o dashboard (`dashboard/`) com três visões
> por persona. 162 testes, todos com IO mockada — a validação com
> credenciais reais (OAuth App, GitHub App/PAT, build Docker) é um débito
> registrado e bloqueante em `docs/validacao-viva.md`.
...
└── docs/
    ...
    ├── seguranca-servico.md    # modelo de acesso, backup do SQLite
...
# dashboard local, sem OAuth (bind restrito a 127.0.0.1, banner de aviso)
